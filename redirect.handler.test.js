...
describe("full integration test", () => {
  describe("root path", () => {
    ...
    test("it redirectst to new url structure", async () => {
      ...
      expect(locationHeader).toEqual('https://winglang.io/docs/');
    });
  });
  ...
  describe("nested path", () => {
    ...
    test("it redirectst to new url structure", async () => {
      const functionOutput = JSON.parse(result.TestResult?.FunctionOutput)
      const locationHeader = functionOutput.response.headers.location.value
      expect(locationHeader).toEqual('https://winglang.io/docs/a-nested-path');
    });
  });
  ...
  describe("a blog path", () => {
    ...
    test("it redirectst to new url structure", async () => {
      const functionOutput = JSON.parse(result.TestResult?.FunctionOutput)
      const locationHeader = functionOutput.response.headers.location.value
      expect(locationHeader).toEqual('https://winglang.io/blog/12/93/a-blog-post');
    });
  });
  ...
  describe("a path with query strings", () => {
    ...
    test("it redirectst to new url structure", async () => {
      const functionOutput = JSON.parse(result.TestResult?.FunctionOutput)
      const locationHeader = functionOutput.response.headers.location.value
      expect(locationHeader).toEqual('https://winglang.io/docs/a-doc-page?foo=bar&baz=qux');
    });
  });
});
